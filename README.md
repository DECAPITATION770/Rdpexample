...
## Устойчивость соединения: WebRTC + HTTP-запасной путь

Видео и управление сначала всегда пробуют идти через WebRTC (P2P,
минимальная задержка). Если за 6 секунд подключиться не удалось — или
если нажать кнопку **⇄** в тулбаре, чтобы переключиться вручную —
браузер сам переходит на запасной HTTP-поток: `GET
/stream/{session_id}`, обычный `multipart/x-mixed-replace` (MJPEG),
без ICE/TURN/UDP — работает через тот же HTTPS-порт, на котором и так
открыта админка. В этом режиме мышь, клавиатура и всплывающие
сообщения идут не через WebRTC DataChannel, а через тот же
сигнальный WebSocket (сообщения типов `input_event`/`overlay_message`).

На каждом участке пути — WebRTC DataChannel для видео (контроль через
`BufferedAmount`), цикл отправки кадров от хоста на сервер, и рассылка
сервером кадров каждому подписчику — устаревший кадр не встаёт в
очередь, а просто отбрасывается. Поэтому картинка не накапливает
задержку даже при плохой сети — вместо этого проседает эффективный
fps.
